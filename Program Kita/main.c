...
/* File */
void openFile (String *s, Tree *tree, Tree tStop, int fileIndex);

/* Similarity Check */
float getSimilarityPercentage (CheckSet checkSet);
float similarityCheck (Tree tree, CheckSet checkSet);
...
//			printf ("\n");
			...
			if (fileIndex != 0 && cekStopword(&(*s), tStop))
				*s = createString(1);
			else
				makeTree(&(*s), &(*tree), fileIndex);
		}
...
float similarityCheck (Tree tree, CheckSet checkSet) {
	 ...
	 */
	 ...
	 travInOrderStat(tree.root, &checkSet);
...
int main() {
	/* Deskripsi Modul :  */
	int fileNumber = 1, idfileStop = 0, finish = 1;
	char select;
...
//	printf ("\nMasukkan nama file stopword : ");
//	gets(s.string);
//	scanf("%[^\n]", s.string);
	strcpy(s.string,"TestStopword.txt");
		
	openFile(&s, &tStop, tStop, idfileStop);
	printf ("\nIni Root : %s \n", tStop.root->kata.string);
	printf ("Size tStop : %d \n\n", size(tStop.root));
//	printInOrder(tStop.root, idfileStop);
...
//		gets(s.string);
		...
		openFile(&s, &tStat, tStop, fileNumber);
//		printf ("\nIni File Number : %d \n", fileNumber);
